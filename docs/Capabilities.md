...
| `rollback [id]` | **The undo reflex** (W3) — every meta/content change snapshots the live page's before-state and is **post-apply verified** (re-fetched to confirm it landed). `rollback <id>` restores that before-state; with no id, lists change types measuring **negative at +28d** (n≥3, CI excludes 0). Autopilot auto-proposes these each cycle (queued in approve mode, capped-executed in auto), and every revert teaches the brain to avoid that change type here | ledger + GSC history | proposals / a revert |
...
| `interview` + identity files | **The employee's persistent self** (OpenClaw workspace pattern + Hermes user modeling): `SOUL.md` (persona/tone/boundaries — **edit your employee's personality with a text editor**), `AGENTS.md` (operating instructions any driving agent reads at session start), `CLIENT.md` (6-question business interview → seeds the brain: the strategist knows the business day one), `MEMORY.md` (auto-curated mirror of the brain, refreshed each cycle), `memory/YYYY-MM-DD.md` (the daily journal each autopilot cycle writes; read today+yesterday at session start), `BOOTSTRAP.md` (one-time first-run ritual — deletes itself when onboarding completes). All create-only: operator edits always win; all injected into persona prompts with char budgets | — | a durable employee, not a stateless tool |
| `profile [--apply]` | **Auto-understand the site, then fix the crawler to match** (LibreCrawl-class, native): platform fingerprinting (WordPress/Webflow/Shopify/Ghost/Wix/HubSpot/Drupal/Next.js/Nuxt/React…), **CSR detection with auto-enabled JS rendering** (or a loud warning if Playwright's missing), robots Crawl-delay honored, crawl sized from the sitemap, **spider mode** (link-following BFS, robots-compliant) when no sitemap exists. Every crawl now computes its own **crawl_depth + inlinks** (deep pages become audit findings) — no proprietary crawler needed. `ingest` auto-profiles on first run | — | site profile + auto-config |
| `sf [--csv\|--crawl]` | *(optional bridge — nothing depends on it)* **Screaming Frog import** — import `Internal:All` exports (CSV/zip/dir): **bootstrap** a metadata corpus with no crawler run, **refresh** (SF-to-SF imports rotate → `sitediff` compares crawls), or **enrich** our crawl (adds crawl depth + inlinks) with a **crawler cross-check** (status/robots/canonical disagreements = cloaking or stale-export findings). Automate the pull: SF's GUI Scheduler exporting into `sf-exports/` (the `agent` daemon auto-imports every tick) or `sf --crawl` for the headless CLI (paid SF license) | SF exports (or SF installed) | corpus + disagreement findings |
