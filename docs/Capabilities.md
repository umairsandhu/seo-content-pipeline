...
| `agent [--interval]` | **Always-on agent mode** (the OpenClaw shape, SEO-only) — one long-running local process: heartbeat, polls review channels, pushes NEW high-sev anomalies to Slack/WhatsApp/email instantly, runs the daily autopilot cycle at your hour, delivers the weekly report. Restart-safe (`state/agent.json`); replaces cron. Phase 2 (roadmap): two-way chat control | — | the local SEO agent |
