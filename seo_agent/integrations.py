...
INTEGRATIONS = [
    ...
    {"key": "ollama", "name": "Ollama (local LLM)", "tier": "optional", "kind": "local-oss",
     "purpose": "Headless drafting/analysis on an OPEN-SOURCE model running on your machine — no cloud key, no data leaves the box",
     "env": [], "config": [], "when": ("llm.provider", "ollama"),
     "unlocks": ["headless draft/retitle/consult on cron", "cloud-free client work"],
     "options": ["`ollama pull llama3.1` · OLLAMA_URL if not localhost", "agent-written (default)", "Anthropic/OpenAI"],
     "docs": "https://ollama.com"},
    {"key": "searxng", "name": "SearXNG (self-hosted SERP)", "tier": "optional", "kind": "local-oss",
     "purpose": "Free open-source SERP fallback when DataForSEO is absent — organic results for briefs/scoring (no PAA/volumes)",
     "env": ["SEARXNG_URL"], "config": [],
     "unlocks": ["brief/score SERP grounding with zero API spend"],
     "options": ["docker run searxng/searxng", "DataForSEO for PAA/volumes/features (recommended)"],
     "docs": "https://docs.searxng.org"},
    {"key": "lighthouse", "name": "Lighthouse CLI (local)", "tier": "optional", "kind": "cli-oss",
     "purpose": "Keyless, quota-free lab Core Web Vitals — the same open-source engine PSI uses, run on your machine (auto-used when no PAGESPEED_API_KEY)",
     "env": [], "config": [],
     "unlocks": ["speed lab data with no key/quota"],
     "options": ["npm i -g lighthouse (or npx)", "PAGESPEED_API_KEY for hosted PSI + CrUX field data"],
     "docs": "https://github.com/GoogleChrome/lighthouse"},
    {"key": "screamingfrog", "name": "Screaming Frog", "tier": "optional", "kind": "file-or-cli",
     "purpose": "Import SF crawls (bootstrap/enrich the corpus + crawler cross-check); auto-pull via SF Scheduler → sf-exports/ or the headless CLI",
     "env": [], "config": [],
     "unlocks": ["sf --csv (import)", "sf --crawl (headless)", "agent auto-import", "sitediff between SF crawls"],
     "options": ["our built-in crawler (`ingest`) — full text; SF adds crawl depth/inlinks + a second opinion"],
     "docs": "https://www.screamingfrog.co.uk/seo-spider/user-guide/general/#command-line"},
